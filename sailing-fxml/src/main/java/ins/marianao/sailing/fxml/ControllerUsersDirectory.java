...
public class ControllerUsersDirectory extends AbstractControllerPDF {
	...
	@FXML
	private TableColumn<User, Number> colIndex;
	@FXML
	...
	@FXML
	private TableColumn<User, Boolean> colUpdate;
	@FXML
	...
	public void initialize(URL url, ResourceBundle resource) {
		...
		this.colIndex.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<User, Number>, ObservableValue<Number>>() {
					@Override
		...
		this.colRole.setCellValueFactory(
		...
		this.colFullname.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<User, String>, ObservableValue<String>>() {
					@Override
					public ObservableValue<String> call(TableColumn.CellDataFeatures<User, String> user) {
						return new SimpleStringProperty(
								user.getValue().isAdmin() ? "" : ((Client) user.getValue()).getFullName());
					}
				});

		this.colPhone.setMinWidth(140);
		this.colPhone.setMaxWidth(180);
		this.colPhone.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<User, String>, ObservableValue<String>>() {
					@Override
					public ObservableValue<String> call(TableColumn.CellDataFeatures<User, String> user) {
						return new SimpleStringProperty(user.getValue().isAdmin() ? ""
								: Formatters.getPhoneFormatter().toString(((Client) user.getValue()).getPhone()));
					}
				});
		...
		this.colUpdate.setCellValueFactory(
		...
		this.colUpdate.setCellFactory(
				new ColumnButton<User, Boolean>(ResourceManager.getInstance().getText("fxml.text.viewUsers.col.update"),
		...
		// define a simple boolean cell value for the action column so that the column
		// will only be shown for non-empty rows.
		this.colDelete.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<User, Boolean>, ObservableValue<Boolean>>() {
					@Override
					public ObservableValue<Boolean> call(TableColumn.CellDataFeatures<User, Boolean> cell) {
						return new SimpleBooleanProperty(false);
		...
		this.colDelete.setCellFactory(
				new ColumnButton<User, Boolean>(ResourceManager.getInstance().getText("fxml.text.viewUsers.col.delete"),
